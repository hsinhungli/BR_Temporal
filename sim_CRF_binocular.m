...
for cond = [1 2]
    ...
    for lay = 1:2
        ...
        subplot(1,2,lay)
        hold on;
        p(cond, lay) = plot(log10(contrasts), rA{lay}, '-o', 'color',[1 0 1]);
        p(cond, lay) = plot(log10(contrasts), rB{lay}, '-o', 'color',[0 0 1]);
    ...
    for lay = 3
        ...
        hold on;
        p(cond, lay) = plot(log10(contrasts), eA{lay}, '-o', 'color',[1 0 1]);
        p(cond, lay) = plot(log10(contrasts), eB{lay}, '-o', 'color',[0 0 1]);
        set(p(cond, lay), 'LineStyle',lstyle);
        set(gca,'YLim', [0 1]);
        xlabel('Contrast','FontSize',14)
        ylabel('Firing Rating','FontSize',14)
    ...
    for lay = 3
        ...
        hold on;
        p(cond, lay) = plot(log10(contrasts), ieA{lay}, '-o', 'color',[1 0 1]);
        p(cond, lay) = plot(log10(contrasts), ieB{lay}, '-o', 'color',[0 0 1]);
        set(p(cond, lay), 'LineStyle',lstyle);
        %set(gca,'YLim', [0 1]);
        xlabel('Contrast','FontSize',14)
        ylabel('Firing Rating','FontSize',14)
